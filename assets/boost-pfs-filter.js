	...
	general: {
      ...
      placeholderProductGridItemClass: 'boost-pfs-filter-product-item boost-pfs-filter-product-item-grid boost-pfs-filter-grid-width-3 boost-pfs-filter-grid-width-mb-2',
		...
		selectOptionContainer: '.boost-pfs-filter-product-item-image',  // CSS selector to append the product option, if left empty it will append to the product item
  		showLimitList: boostPFSThemeConfig.custom.customProductsPerPageOptions
    },
	selector: {
...
(function() {
		...
		if (data) {
          ...
          // console.log(data)
      			if (data.compare_at_price_min) {
      				...
      				if (priceVaries) {
      					html += '<span class="boost-pfs-filter-product-item-price-from-text">' + 'From' + ' ' + '</span>';
      				}
      				var priceMin = Math.ceil(data.price_min);
                	html += '<span class="boost-pfs-filter-product-item-regular-price">' + Utils.formatMoney(priceMin) + '</span>';
      			}              
              html += '</p>';	
          }
        return html;
	...
	function buildLabels(data) {
		...
		if (/*boostPFSThemeConfig.custom.hasOwnProperty('sold_out_enable') &&
			boostPFSThemeConfig.custom.sold_out_enable &&*/ soldOut) {
          	soldOutLabel = '<span class="soldout boost-pfs-filter-label boost-pfs-filter-label-display-text"> Sold out </span>'
  ...
  ProductDisplayType.prototype.render = function() {
	...
	ProductLimit.prototype.compileTemplate = function() {
		var html = '';
      	var limit = Globals.queryParams.limit 
		if (boostPFSTemplate.hasOwnProperty('showLimitHtml')) {
			...
			if (numberList != '') {
                ...
                for (var k in arr) {
                  if(typeof arr[k] === 'string' && !isNaN(parseFloat(arr[k])) && parseFloat(arr[k]) <= 70){
                    var page = arr[k].trim();
                    var activeGlass = limit == page ? "boost-active-per-page" : "";
                    showLimitItemsHtml += '<div class="boost-custom-limit '+ activeGlass +'" data-page="'+page+'"><span>'+page+'</span></div>';
                  }	
                }
...
// 			$topLimitSelect.val(Utils.stripHtml(Globals.queryParams.limit));
// 		}
	}

  
  	ProductLimit.prototype.bindEvents = function() {
        jQ(Selector.topShowLimit + ' .boost-custom-limit').on("click" , function(){
            FilterApi.setParam('limit', Utils.stripHtml(jQ(this).data("page")));
            FilterApi.setParam('page', 1);
